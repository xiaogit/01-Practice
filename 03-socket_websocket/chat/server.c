  ...
  pthread_t thid;
    ...
    while (1)  
      {  
	    ...
	    pthread_create(&thid, NULL, thread, &connfd);
	    printf("connfd: %d\n", connfd);
            continue;  
          }  
...
void * thread(void * vargp)
{
	...
	while(1)
	{
		...
		ret = select(1, &fds, NULL, NULL, 0);
		if(ret == -1)
		{
			printf("select %s:%d",__FILE__, __LINE__);
			break;
		}
		else if( ret == 0)
			continue;
